...
def compare_values_in_hsv(img_original, img_attack, img_fingerprint):
    ...
    :param img_original: the roi of the original image
    :param img_attack: the roi of the attack image
    :param img_fingerprint: the corresponding binariesed fingerprint of the original
    :return:
    """
    img = img_attack
    ...
    original_bright = np.round((fingerprint_AND_Original_HSV[:,:,2].mean()/np.count_nonzero(fingerprint)*291600))
    print("Bright Spots Original:", original_bright)
    original_dark = np.round((fingerprint_inverted_And_original_HSV[:,:,2].mean()/np.count_nonzero(fingerprint_inverted)*291600))
